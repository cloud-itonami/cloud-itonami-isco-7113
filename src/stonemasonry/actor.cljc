(ns stonemasonry.actor
  "StonemasonryActor — the ISCO-08 7113 stonemasonry workshop
  coordination actor as a `langgraph.graph/state-graph` (ADR-2607121000
  / CLAUDE.md Actors section). One graph run = one workshop-
  ...
  an interrupted run can resume after human sign-off (e.g. a workshop
  safety officer's review of a flagged concern, or a supply-order
  budget approval). Modeled on cloud-itonami-isco-3313's
  ...
  The unconditional invariant: the Workshop Coordination Advisor can
  never directly commit a record the StonemasonryGovernor refuses —
  ...
  stonework itself, never dispatches hardware, and never finalizes a
...
(defn build-graph
  "Build a compiled StonemasonryActor graph. `store` implements
  `stonemasonry.store/Store`. `advisor` implements
  `stonemasonry.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:workshop-id (:workshop-id request)
                                    :op (:op proposal)
                                    :craftsperson-id (:craftsperson-id proposal)
                                    :payload proposal}]
